/**
 * The three panes that are not a file.
 *
 * The variable manager, the collection runner and the settings pane are workspace-wide or
 * app-wide tools, not documents, so they are an overlay over the editor area rather than entries
 * in the tab strip. A tab is a file with unsaved bytes, a title, a dirty dot and a draft in app
 * data; teaching `tabsStore` that some of its tabs have none of those would put a `kind` check in
 * every one of those paths to buy nothing.
 *
 * One at a time, deliberately. All three are full-width and all three are things you open, use and
 * close; two of them side by side would mean laying out a pane arrangement nobody asked for.
 *
...
export type Overlay =
  { readonly kind: "variables" } | { readonly kind: "runner"; readonly nodeId: string } | { readonly kind: "settings" };
